...
В `examples` сохраняются полная строка, Loki timestamp, result labels и явная metadata.
...
Полный бюджет сериализованного MCP-ответа ещё не применяется (S06). В частности,
несколько крупных исходных примеров могут превысить `maxResponseBytes`; размер каждого
HTTP-ответа уже ограничен. `fields`-проекция, кеш, `entryId` и курсоры пока не реализованы.
