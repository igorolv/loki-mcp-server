...
- До 10 значений метки, остальное `(+N more: discoverLogs with label="<имя>")`; больше 50
  различных значений — «high cardinality, not listed». Значения обрезаются до 60 символов.
...
- `Levels seen` — значения уровня из меток, structured metadata или строки в верхнем регистре;
  `detected_level="unknown"` Loki 3.x считается отсутствием уровня.
