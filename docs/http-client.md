...
`LokiHttpClient` — внутренний транспортный слой для следующих пакетов. Публичный
MCP-каталог пока содержит только `listConnections`. Новых tools в S03 нет.
...
Относительное время и timezone обрабатываются будущим прикладным слоем S04.
...
бюджет всего MCP-ответа — к S06. `queryRange` передаёт явно полученный `limit`
...
недоступным. MCP-обёртка этих ошибок будет проверена с data tools в S04.
...
Контейнерная совместимость Loki 2.6.1 и 3.x остаётся проверкой S04; live-проверки
в S03 не запускались и для этого пакета не требуются.
