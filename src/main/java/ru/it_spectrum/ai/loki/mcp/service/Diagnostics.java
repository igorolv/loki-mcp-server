...
public final class Diagnostics {
    ...
    public static String enter(String connection) {
        ...
        if (connection == null || connection.isBlank()) MDC.remove(MDC_CONNECTION);
        else MDC.put(MDC_CONNECTION, connection);
        return previous;
    }

    public static void leave(String previous) {
        if (previous == null) MDC.remove(MDC_CONNECTION);
        else MDC.put(MDC_CONNECTION, previous);
    }
}
