...
public class DiscoveryService {
    ...
    private String labelsOverview(String connection, QueryTime.Range window, ConnectionDefinition definition,
        ...
        if (names.size() > listed.size()) text.append(" Showing the first ").append(listed.size()).append(" label names.");
        appendLabels(text, labels);
    ...
    private static void appendLabels(StringBuilder text, Map<String, Set<String>> labels) {
        if (labels.isEmpty()) { text.append("\nNo labels found in this window."); return; }
        text.append("\nLabels:");
        for (var label : labels.entrySet()) {
            ...
            else if (values.size() > VALUES_PER_LABEL) text.append(values.size()).append(" distinct values (high cardinality, not listed)");
            else {
    ...
    private Sample sample(String connection, String scope, QueryTime.Range window, ConnectionDefinition definition) {
        ...
        try { response = client.queryRange(connection, scope, window.start(), window.end(), limit, LokiHttpClient.Direction.BACKWARD, null); }
        catch (LokiOperationException error) {
            if (error.error().code() == ErrorCode.OPERATION_CANCELLED) throw error;
            return new Sample(List.of(), 0, 0, Map.of(), Set.of());
        }
        if (!(response.data() instanceof LokiResponses.Streams streams)) return new Sample(List.of(), 0, 0, Map.of(), Set.of());
        ...
        for (var event : events) {
            ...
            if (view.format() == EventNormalizer.Format.JSON) json++; else plain++;
            for (String path : view.jsonFields().keySet()) fields.merge(EventNormalizer.lokiFieldName(path), 1, Integer::sum);
    ...
    private static void appendSample(StringBuilder text, Sample sample, int exampleChars) {
        if (sample.events().isEmpty()) { text.append("\nNo lines sampled in this window; fields are unknown."); return; }
        ...
        if (!sample.fields().isEmpty()) {
            ...
            if (sample.fields().size() > names.size()) text.append(" (+").append(sample.fields().size() - names.size()).append(" more)");
            text.append('.');
        }
        if (exampleChars > 0) text.append("\nExample line: ").append(truncate(sample.events().getFirst().line().replace('\n', ' '), exampleChars));
    }
    ...
    private static void appendNext(StringBuilder text, String scope, Map<String, Set<String>> labels, Sample sample, ConnectionDefinition definition) {
        ...
        if (levelField != null) text.append("; filter JSON fields with | json, e.g. | json | ").append(levelField).append("=~\"(?i)error\"");
