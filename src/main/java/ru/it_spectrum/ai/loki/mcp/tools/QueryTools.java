...
import ru.it_spectrum.ai.loki.mcp.model.QueryResults.Logs;
...
public class QueryTools {
    ...
    @McpTool(name = "queryLogs", generateOutputSchema = true,
            description = "Search Loki with arbitrary log LogQL in an explicit time window. Returns globally ordered query-output lines, "
                    + "result labels, nanosecond timestamps and completeness. A reached limit means more matches are unknown; no cursors yet. "
    ...
    public Logs queryLogs(
        ...
        return service.logs(connection, query, start, end, direction, limit);
    }
